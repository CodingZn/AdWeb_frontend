...
import { DISTANCE, Moveable } from "../utils/Moveable";
...
export class NPC extends Character {
  ...
  public override move(dt: number, colliders?: Iterable<Renderable>): { x: number; y: number; z: number; } {
    ...
    if (z !== 0) {
      ...
      let intersect = this.collide(colliders, dir, DISTANCE)
      if (intersect !== null) {
        // todo 更加智能
        // 目前遇到障碍随机转身
    ...
    let intersect = Moveable.collide(pos, new Vector3(0, -1, 0), colliders, Infinity);
    let y = 0;
    if (intersect !== null) {
